...
def extract_keywords(
    ...
    keywords = [
        candidate.keyword
        for candidate in sorted_candidates[:MAX_KEYWORDS]
    ]
...
def _add_active_window_candidates(
    ...
    for keyword in window_keywords:
        weight = 3.0 if keyword != "folder" else 2.5
...
def _extract_nouns(text: str) -> list[str]:
    try:
        ...
        return [
            noun
            for noun in nouns
            if noun and noun not in stop_words and len(noun) > 2
        ][:20]
    except (ImportError, LookupError):
        LOGGER.debug("NLTK noun extraction unavailable; using regex fallback")

    return [
        token
        for token in _tokenize(text)
        if token not in FALLBACK_STOPWORDS and len(token) > 3
    ][:20]
